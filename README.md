...
## 🗂️ Cấu trúc S3 Bucket `anomalytraffic`
...
```
Nguồn anomaly  → Dự đoán Benign   ⚠️  CONFLICT (model bỏ sót tấn công)
Nguồn log      → Dự đoán Attack   ⚠️  CONFLICT (model báo nhầm lưu lượng bình thường)
```
...
|---|---|---|---|
| `attack` | `Benign` | `attack_needs_review` | `low` ⚠️ Cần review thủ công |
| `Benign` | Bất kỳ attack | `Benign` | `high` ✅ Tự động sửa |
| Khác | Khác | Giữ expected | `low` ⚠️ Fallback |
...
> ⚠️ **Lưu ý quan trọng về data chưng cất:**
