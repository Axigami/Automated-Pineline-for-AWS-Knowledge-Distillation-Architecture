param (
    ...
    [string]$ImageName = "distillation-student",
...
Write-Host " distillation-student   Build & Push to ECR" -ForegroundColor Cyan
Write-Host "=============================================" -ForegroundColor Cyan
Write-Host "Account ID  : $AwsAccountId"
Write-Host "Region      : $AwsRegion"
Write-Host "Image URL   : ${RepositoryUri}:${ImageTag}"
Write-Host "Script      : IOT-PROJECT.py"
Write-Host "=============================================" -ForegroundColor Cyan
...
# Ensure IOT-PROJECT.py is in the distill/ folder
if (-not (Test-Path ".\distill\IOT-PROJECT.py")) {
    Write-Host "ERROR: .\distill\IOT-PROJECT.py not found!" -ForegroundColor Red
    exit 1
}

docker build --provenance=false -t ${ImageName}:${ImageTag} .\distill
if ($LASTEXITCODE -ne 0) {
...
Write-Host "ECR_IMAGE_DISTILL = ${RepositoryUri}:${ImageTag}" -ForegroundColor Yellow
