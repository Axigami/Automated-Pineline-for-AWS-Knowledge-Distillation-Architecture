param (
    ...
    [string]$ImageName = "export-onnx-lambda",
...
Write-Host " AWS Lambda Container Image Build & Push" -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
...
# 2. Check if ECR repo exists, create if not
...
if (-not (Test-Path ".\ExportONNX.py")) {
    Copy-Item "..\Lambda code\ExportONNX.py" ".\ExportONNX.py" -Force
}
docker build --provenance=false -t ${ImageName}:${ImageTag} .
if ($LASTEXITCODE -ne 0) {
...
Write-Host "Image successfully pushed to ECR:"
Write-Host "${RepositoryUri}:${ImageTag}"
