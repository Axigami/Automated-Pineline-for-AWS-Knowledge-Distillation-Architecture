...
PREPARE_DATA_FUNCTION = os.environ.get('PREPARE_DATA_FUNCTION', 'PrepareDistillationData')
...
def lambda_handler(event, context):
    ...
    if len(conflicts) == 0:
        return {'statusCode': 200, 'relabeled_count': 0, 'message': 'No pending conflicts'}

    # ── Relabel each conflict ────────────────────────────────────────────────
    relabeled_count     = 0
    high_conf_count     = 0
    low_conf_count      = 0
    ...
    for conflict in conflicts:
        try:
            ...
            table.update_item(
                Key={'conflict_id': conflict_id, 'created_at': created_at},
                UpdateExpression='''
                    SET #status = :status,
                        correct_label = :label,
    ...
    # ── Trigger PrepareDistillationData nếu có high-confidence ──────────────
    if high_conf_count > 0:
        print(f"🚀 Triggering {PREPARE_DATA_FUNCTION} ({high_conf_count} high-conf samples)...")
        try:
            lambda_client.invoke(
                FunctionName=PREPARE_DATA_FUNCTION,
                InvocationType='Event',
                Payload=json.dumps({
                    'triggered_by':    'Relabel',
                    'high_conf_count': high_conf_count
                })
            )
            print(f"✅ Triggered {PREPARE_DATA_FUNCTION}")
        except Exception as e:
            print(f"❌ Failed to trigger {PREPARE_DATA_FUNCTION}: {e}")
    else:
        print("⚠️ No high-confidence labels — skipping PrepareDistillationData")
